...
## the Ip of Nao
#################################################################################
def InitProxy(IP="marvin.local", proxy=[0]):
...
###############################################################################
def EyeLED(color=[0,0,0],interpol_time = 0):
    sGroup = "FaceLeds"
...
def LoadDialog(file_name):
    ...
    csv_reader = csv.reader(file_load)
    return csv_reader
...
def Stiffen(stiffness = True):
    """Make Nao stiffen its joints (Can be True or False)"""
    motionProxy.stiffnessInterpolation('Body',int(stiffness), 1)
...
def Play(file_name):
    ...
    playProxy.post.playFile(file_name)
...
##    ###########
    ...
    offset=-0.2
    gain=1.5
    yaw = [0.0,0.0,-0.2,-0.2,-0.2,0,0.2,0.2,0.2,0.2,0.0,-0.2,-0.4,-0.4,-0.4,-0.4,
           -0.4,-0.2,0.0,0.2,0.4,0.4,0.4,0.4,0.4,0]
...
## Allows you to set rgb, ledgroup, duration and frequency.
#########################################################################
def RunLED(file_name, post = True):
    """ Give up the filename containing the movement. Needs led proxy."""
    list_path = sys.path
    filefound=False
    for i in range (0,len(list_path)):
    ...
    ledProxy.post.off(parameters[0])

    rgb = 256*256*int(parameters[1])+256*int(parameters[2])+int(parameters[3])

    # in case there is no frequency provided led are on. If frequency is too low as well.
    if (float(parameters[4]) == 0) or (1.0/float(parameters[4]) >= float(parameters[5])):
        rgb_all = [rgb, rgb, 0]

        ledProxy.post.fadeListRGB(parameters[0],
                              rgb_all,
                              [0.1,int(parameters[5]),0.1])
                              
    else:
        halfperiod = float(1.0/(float(parameters[4])*2.0))
        rgb_all = [rgb, 0]
        times = [halfperiod, halfperiod]
        rgb_all_long = []
        times_long = []

        # repeat light pattern for a few times depending on the set length
        for i in range(int(5/sum(times))):
            rgb_all_long.extend(rgb_all)
            times_long.extend(times)

        ledProxy.post.fadeListRGB("FaceLeds",
                                  rgb_all_long,
                                  times_long)
    return file_load
