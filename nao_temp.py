...
## the Ip of Nao
#################################################################################
def InitProxyName(proxy_name, IP, PORT):
    """Tries to initialize a proxy named proxy_name at a specified IP and  PORT."""
    try:
        theproxy = naoqi.ALProxy(proxy_name, IP, PORT)
        sleep(0.01)
        #print "Proxy ", proxy_name, " established."
        return theproxy
    except RuntimeError as e:
        print "Unable to create proxy: ", proxy_name
        print str(e)
        return None
...
def InitProxy(IP="marvin.local", proxy=[0], PORT = 9559):
    ...
    proxy_dict={1:"ALTextToSpeech",2:"ALAudioDevice",3:"ALMotion",
                4:"ALMemory",5:"ALFaceDetection",6:"ALVideoDevice",
                7:"ALLeds",8:"ALFaceTracker",9:"ALSpeechRecognition",
                10:"ALAudioPlayer",11:"ALVisionToolbox",12:"ALSonar"}
    
    proxy_list={1:"tts",2:"audioProxy",3:"motionProxy",
                4:"memoryProxy",5:"faceProxy",6:"cameraProxy",
                7:"ledProxy",8:"trackfaceProxy",9:"speechProxy",
                10:"playProxy",11:"videoProxy",12:"alsonarProxy"}
...
##    global playProxy
##    global videoProxy # VisionToolbox is deprecated
##    global speechProxy
##    global alsonarProxy

    if 'proxy' not in locals():
        all_proxy = True
    elif not type(proxy) is list:
        all_proxy = True
    elif len(proxy)==0:
        all_proxy = True
    elif 0 in proxy:
    ...
    for p in proxy:
        globals()[proxy_list[p]]=InitProxyName(proxy_dict[p],IP, PORT)

    
    
## All the AL Modules from 1.14 documentation
...
def InitProxyOld(IP="marvin.local", proxy=[0], PORT = 9559):
    """proxy: (list) 1->TTS, 2->audio, 3->motion, 4->memory, 5->face, 6->video, 7->LED's, 8->Track, 9->Speech, 10->Audioplayer, 11->VisionToolbox"""
    global audioProxy
    global motionProxy
    global memoryProxy
